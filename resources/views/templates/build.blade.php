...
<span class="cm-W">{{ ($log_items->is_distance) ? Format::format_distance($exercise_values[$log_items->logtempitem_id]) : ($log_items->is_time ? Format::format_time($exercise_values[$log_items->logtempitem_id]) : Format::format_weight($exercise_values[$log_items->logtempitem_id])) }}</span>{!! ($log_items->logtempitem_reps > 1 || $log_items->logtempitem_sets > 1) ? '<span class="cm-R"> x ' . $log_items->logtempitem_reps . '</span>' : '' !!}{!! ($log_items->logtempitem_sets > 1) ? '<span class="cm-S"> x ' . $log_items->logtempitem_sets . '</span>' : '' !!}{!! ($log_items->is_rpe) ? '<span class="cm-RPE"> @' . $log_items->logtempitem_rpe . '</span>' : '' !!}{!! ($log_items->logtempitem_comment != '') ? '<span class="cm-C"> ' . $log_items->logtempitem_comment . '</span>' : '' !!}
@endforeach
